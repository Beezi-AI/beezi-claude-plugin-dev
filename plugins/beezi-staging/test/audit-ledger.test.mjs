...
function makeHome(t) {
  ...
  fs.mkdirSync(path.join(dir, 'accounts', 'a1b2c3d4'), { recursive: true });
...
test('1. an empty ledger reports nothing as imported', (t) => {
  makeHome(t);
  const ledger = loadLedger('a1b2c3d4');
...
test('2. round-trips a marked session through disk', (t) => {
  ...
  saveLedger('a1b2c3d4', ledger);

  const reloaded = loadLedger('a1b2c3d4');
...
test('3. a rejected session counts as imported', (t) => {
  ...
  assert.equal(isImported(loadLedger('a1b2c3d4'), 'sess-1'), true);
});

test('4. a ledger with an unknown version is discarded, not merged', (t) => {
  makeHome(t);
  fs.writeFileSync(
    auditLedgerFile('a1b2c3d4'),
    JSON.stringify({ version: 99, sessions: { 'sess-1': {} } }),
    'utf-8',
  );

  assert.equal(isImported(loadLedger('a1b2c3d4'), 'sess-1'), false);
});

test('5. a corrupt ledger file falls back to empty instead of throwing', (t) => {
  makeHome(t);
  fs.mkdirSync(path.dirname(auditLedgerFile('a1b2c3d4')), { recursive: true });
  fs.writeFileSync(auditLedgerFile('a1b2c3d4'), 'not json at all', 'utf-8');

  assert.deepEqual(loadLedger('a1b2c3d4').sessions, {});
});
...
test('6. survives pruneStale — the ledger is outside state/ and queue/', (t) => {
  ...
  fs.utimesSync(auditLedgerFile('a1b2c3d4'), ancient, ancient);
  ...
  assert.equal(isImported(loadLedger('a1b2c3d4'), 'sess-1'), true, 'ledger survived');
});

test('7. the ledger file sits at the account root', (t) => {
  const home = makeHome(t);

  assert.equal(auditLedgerFile('a1b2c3d4'), path.join(home, 'accounts', 'a1b2c3d4', 'audit-ledger.json'));
});
...
test('7. the complete flag round-trips and survives a reload', (t) => {
  makeHome(t);
  const ledger = loadLedger('a1b2c3d4', 'client-1');
  assert.equal(isComplete(ledger), false);

  markComplete(ledger);
  saveLedger('a1b2c3d4', ledger);

  const reloaded = loadLedger('a1b2c3d4', 'client-1');
...
test('8. a foreign-identity ledger is discarded on load', (t) => {
  makeHome(t);
  const original = loadLedger('a1b2c3d4', 'client-a');
  markImported(original, 'sess-1', { outcome: 'accepted', reports: 3 });
  markComplete(original);
  saveLedger('a1b2c3d4', original);

  const foreign = loadLedger('a1b2c3d4', 'client-b');
  ...
  const same = loadLedger('a1b2c3d4', 'client-a');
...
test('9. an identity-less ledger is adopted by the first identified load', (t) => {
  makeHome(t);
  const legacy = loadLedger('a1b2c3d4');
  markImported(legacy, 'sess-1', { outcome: 'accepted' });
  saveLedger('a1b2c3d4', legacy);

  const adopted = loadLedger('a1b2c3d4', 'client-a');
