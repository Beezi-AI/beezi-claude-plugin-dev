...
test('whoami — 200 with body → valid with fields', async () => {
  const res = await whoami({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => ({
    ok: true,
    json: async () => ({
      email: 'dev@acme.com',
      name: 'Dev Eloper',
...
test('whoami — 401 → { valid: false }', async () => {
  const res = await whoami({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => ({ status: 401, ok: false })));
  assert.deepEqual(res, { valid: false });
});

test('whoami — 403 → { valid: false }', async () => {
  const res = await whoami({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => ({ status: 403, ok: false })));
  assert.deepEqual(res, { valid: false });
});

test('whoami — other non-ok (500) → null', async () => {
  const res = await whoami({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => ({ status: 500, ok: false })));
  assert.equal(res, null);
});

test('whoami — fetch throws (offline) → null', async () => {
  const res = await whoami({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => { throw new Error('ECONNREFUSED'); }));
  assert.equal(res, null);
});
...
test('whoami — 200 but body missing fields → nulls', async () => {
  const res = await whoami({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => ({ ok: true, json: async () => ({}) })));
  assert.deepEqual(res, {
    valid: true,
    email: null,
    name: null,
...
test('whoami — sends bearer token to the whoami URL', async () => {
  let seen;
  await whoami({ key: 'a1b2c3d4', clientId: 'client-a', token: 'my-token' }, deps(async (url, opts) => {
...
test('probeIdentity — 401 is unauthorized, 403 is forbidden, and both keep their status', async () => {
  const unauthorized = await probeIdentity({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => ({ status: 401, ok: false })));
  assert.equal(unauthorized.outcome, PROBE_OUTCOMES.UNAUTHORIZED);
  assert.equal(unauthorized.httpStatus, 401);
  const forbidden = await probeIdentity({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => ({ status: 403, ok: false })));
...
test('probeIdentity — 503 OAUTH_VERIFICATION_UNAVAILABLE is unavailable, never invalid', async () => {
  const res = await probeIdentity({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => ({
...
test('probeIdentity — a transport failure is unavailable with no status', async () => {
  const res = await probeIdentity({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => { throw new Error('ECONNREFUSED'); }));
  assert.equal(res.outcome, PROBE_OUTCOMES.UNAVAILABLE);
  assert.equal(res.httpStatus, null);
});

test('probeIdentity — 429 is unavailable and keeps its status', async () => {
  const res = await probeIdentity({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => ({ status: 429, ok: false })));
  assert.equal(res.outcome, PROBE_OUTCOMES.UNAVAILABLE);
...
test('probeIdentity — an HTTP failure carries a status, so it carries no probe_unreachable', async () => {
  const res = await probeIdentity({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => ({ status: 500, ok: false })));
...
test('probeIdentity — an ordinary 5xx carries no reason of its own', async () => {
  const res = await probeIdentity({ key: 'a1b2c3d4', clientId: 'client-a', token: 'tok' }, deps(async () => ({ status: 502, ok: false })));
  assert.equal(res.reason, null);
});

test('probeIdentity — 401 and 403 name their own verdicts', async () => {
  assert.equal((await probeIdentity({ key: 'a1b2c3d4', clientId: 'client-a', token: 't' }, deps(async () => ({ status: 401, ok: false })))).reason, 'unauthorized');
  assert.equal((await probeIdentity({ key: 'a1b2c3d4', clientId: 'client-a', token: 't' }, deps(async () => ({ status: 403, ok: false })))).reason, 'forbidden');
});
